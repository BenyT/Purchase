...
@property (nonatomic ,weak) BAImageView *cartImage;
...
- (void)viewDidLoad {
    ...
    BAImageView *cartImage =[[BAImageView alloc]initWithFrame:(CGRect){10,5,40,40}];
    cartImage.image=[UIImage imageNamed:@"Home_Cart.jpg"];
    [cartImage addTarget:self action:@selector(cartImageClick) forControlEvents:BAImageViewControlEventTap];
...
#warning 这里是判断当前购物车的价格是否低于预消费，如果不需要改成大于0 里面填写自己的按钮text
    ...
    NSNumberFormatter *formatter =[[NSNumberFormatter alloc]init];
    formatter.numberStyle=NSNumberFormatterDecimalStyle;
    _totalPrice.text=[NSString stringWithFormat:@"￥%@",[formatter stringFromNumber:[NSNumber numberWithDouble:total]]];
...
-(void)cartImageClick
{
    ...
    {
        TableOfContentsModal *vc =[[TableOfContentsModal alloc]init];
        vc.TableOfContentsDelegate=self;
        __weak typeof(self) weakSelf =self;
        vc.TableOfContentsBlock=^{
            [weakSelf.rightTableView reloadData];
        };
        vc.tapLabelBlock=^(NSMutableArray *array){
            //删除字典所有对象
            ...
            for (int i=0; i<[_dockArray count]; i++) {
                NSMutableArray *arrayDock =_dockArray[i][@"right"];
